...
In this occasion, it is all about Large Language Models, and how Apache Camel brings a ton of new and very cool functionality allowing you to create data flows and integrations to implement LLM-based services.
...
This tutorial managed to include a tiny LLM with tool support (function calling) gifting you the chance to try out very interesting interactions with the model. You will deploy the LLM and you will create processing flows using Apache Camel connected to the LLM. 
...
- Discover new functionality in Camel to implement **AI** use cases.
- Better understand LLMs and how to leverage the use of **tools**.
- Try out the **Kaoto** UI to graphically create Camel routes.
- Experience how **Camel JBang** accelerates your prototyping phase.
- Learn how to use brand new features in Camel JBang like:
** its brand new **kubernetes plugin** to deploy Camel in OpenShift.
...
- *step* (jump to step)
- *chat* (curl-based script to interact with Camel)
...
Toggle (✓) auto-save in your editor, as illustrated below, to automatically trigger hot-reloads.
+
...
We provide a Java class with the base initialisation you need to get started.

There are two key parts to understand.

- The first is the configuration to connect to the LLM to interact with:
+
----
...
- The second is the prompt we define to instruct the LLM how to respond.
+
----
...
At this point, the code does not include any Camel process definition yet. Your first task is to define a very simple Camel route.

{empty} +

=== Create your Camel route using Kaoto.

Kaoto is a graphical UI that will help you to build Camel routes using an intuitive user interface helping you to learn about Camel.

[TIP]
.⏩ *Fast-Forward* 
====
Only if you want to skip (automate) the creation process, execute in your terminal the command below:
...
. Then <<test-first-route,click here to jump into the next section>>.
====
...
image::images/10-kaoto-full-route.png[width=40%, align=center]
...
Your `lab` folder contains now a Camel route in `routes.yaml` able to listen to HTTP requests.

[NOTE]
====
If the route is not already running, use Camel JBang to start it:
...
Split your terminal by clicking the button as per the image below:
