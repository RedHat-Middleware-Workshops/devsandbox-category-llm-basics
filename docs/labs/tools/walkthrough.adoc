...
Follow the instructions below to enable the scripts and setup the lab.
====
...
Now you should find, under the `lab` directory, find your initial source files:
...
. Then <<test-first-route,click here to bypass the Kaoto instructions and jump to the next section "*_Talk to your LLM_*">>.
====
...
[source, subs=]
----
source $LLM_TOOLS/scripts/setup
chat hello<br>
----
====
...
The _Apache Camel_ langchain4j component makes it very easy to create tools the LLM can consume. 
...
To speed up the implementation process, fast-forward as indicated below to auto-generate the _Camel_ route process.
