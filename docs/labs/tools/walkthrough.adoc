...
In this final stage of the tutorial you will take your prototype, create a project out of it, and deploy it OpenShift.

You will see how the tooling,(Camel JBang), makes this process very smooth having not to worry about the packaging and deployment processes.

NOTE: Camel JBang is continuously being improved and perfected. There are currently a number of raised tickets to make the whole experience seamless. In the meantime you'll have to perform a number of manual steps while waiting for the release of the enhancements in Camel JBang.

{empty} +

=== Export step

All this time Camel JBang allowed you to work on your prototype with strictly the source files that were necessary.

Now, as a pre-condition to deploy on OpenShift you'll need to structure the files into a Maven project in order to packaged and containerise the solution.

Execute in your terminal the following command to convert your Camel JBang prototype into a Quarkus project:
...
--ignore-loading-error<br>
----

NOTE: The export command above require a couple of explicit needed dependencies to run LangChain4j using the Quarkus libraries.

{blank}

Then, run the following command to eliminate a parameter that Quarkus does not understand:
...
NOTE: This is a temporary workaround needed to eliminate a parameter Quarkus does not understand on its current release. This will be unnecessary in subsequent Quarkus releases.
